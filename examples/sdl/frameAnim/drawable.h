...
public:
  ...
  virtual bool gotShot(const Drawable*) const { 
    std::cout << "I can't be shot: "+getName() << std::endl; 
    return false;
  }
  virtual bool doneDying() const { 
    std::cout << "I can't be done dying: "+getName() << std::endl; 
    return false;
  }
  virtual void shoot() { 
    std::cout << "I don't shoot: "+getName() << std::endl; 
  }
  virtual bool collidedWith( const Drawable* ) = 0;
  virtual void explode() { 
    std::cout << "I don't explode: "+getName() << std::endl; 
  }
